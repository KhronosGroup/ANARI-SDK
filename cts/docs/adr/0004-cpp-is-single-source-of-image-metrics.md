...
metric deltas, and skip/feature differences). Rendering duration is excluded
from semantic equality so timing-only variation does not make repeated runs
differ. There is exactly one metric implementation.
