...
def get_channels(parsed_json):
    ...
    if "sceneParameters" in parsed_json:
        ...
        if (
            "frame_color_type" in scene_parameters
            and scene_parameters["frame_color_type"] != ""
        ):
            channels.append("color")
        if (
            "frame_depth_type" in scene_parameters
            and scene_parameters["frame_depth_type"] != ""
        ):
...
def globImages(directory, prefix="", exclude_prefix=""):
    ...
    if exclude_prefix != "":
        path_list = [
            path for path in path_list if not Path(path).name.startswith(exclude_prefix)
        ]
    return path_list
                ...
                if isinstance(value, dict):
                    for channel, channelValue in value.items():
                        if isinstance(channelValue, dict):
                            if (
                                "missingImage" in evaluation[stem][name][channel]
                                and evaluation[stem][name][channel]["missingImage"]
                            ):
                                continue
                            ...
                            reference_image_path = (
                                Path("reference") / f"{name}_{channel}.png"
                            )
                            ctsUtility.write_image(
                                output_path / reference_image_path,
                                evaluation[stem][name][channel]["images"]["reference"],
                            )
                            evaluation[stem][name][channel]["image_paths"][
                                "reference"
                            ] = reference_image_path
                            ...
                            candidate_image_path = (
                                Path("candidate") / f"{name}_{channel}.png"
                            )
                            ctsUtility.write_image(
                                output_path / candidate_image_path,
                                evaluation[stem][name][channel]["images"]["candidate"],
                            )
                            evaluation[stem][name][channel]["image_paths"][
                                "candidate"
                            ] = candidate_image_path
                            ...
                            ctsUtility.write_image(
                                ...
                                evaluation[stem][name][channel]["images"]["diff"],
                            ...
                            thresholds_image_path = (
                                Path("thresholds") / f"{name}_{channel}.png"
                            )
                            ctsUtility.write_image(
                                output_path / thresholds_image_path,
                                evaluation[stem][name][channel]["images"]["threshold"],
                                check_contrast=False,
                            )
                            evaluation[stem][name][channel]["image_paths"][
                                "threshold"
...
def write_report(results, output, check_features=True, verbosity=0):
    ...
    title = (
        f"CTS - Report"
        if "renderer" not in results
        else f'CTS - Library: {results["library"]} Device: {results["device"]} Renderer: {results["renderer"]}'
    )
    ctsReport.generate_report_document(
        results, output_path, title, check_features, verbosity
    )
...
def evaluate_scene(
    ...
    parsedThresholds = {}
    if (
        "thresholds" in parsed_json
        and isinstance(parsed_json["thresholds"], dict)
        and parsed_json["thresholds"]
    ):
    ...
    for channel in channels:
        ...
        if ref_path == "":
            print(
                "No reference image for filepath {} could be found.".format(
                    reference_file
                )
            )
            continue
        ...
        if candidate_path == "":
            print(
                "No candidate images for filepath {} could be found.".format(
                    candidate_file
                )
            )
            results[str(test_name)][name][channel] = {"missingImage": True}
            continue
        ...
        eval_result = ctsUtility.evaluate_scene(
            ref_path,
            candidate_path,
            methodsCopy,
            channelThresholds,
        ...
        for key in eval_result["metrics"]:
            ...
            hasPassed = (
                ...
                if eval_result["passed"][key]
                else "\033[91mFailed\033[0m"
            )
            print(
                f'   {key}: {hasPassed} {eval_result["metrics"][key]} Threshold: {eval_result["thresholds"][key]}'
            )
...
def resolve_scenes(test_scenes):
    ...
    if isinstance(test_scenes, list):
        ...
        collected_scenes = [
            n for n in collected_scenes if fnmatch.fnmatch(str(n), "*.json")
        ]
        if collected_scenes == []:
...
def render_scene(
    ...
    if "color" in channels and image_data_list[0]:
        ...
        outName = file_name.with_suffix(".png").with_stem(
            f"{prefix}{stem}{permutationString}_color"
        )
    ...
    if "depth" in channels and image_data_list[1]:
        ...
        image_out = ImageOps.flip(image_out)
        outName = file_name.with_suffix(".png").with_stem(
            f"{prefix}{stem}{permutationString}_depth"
        )
        print(f"   Rendering to {outName.resolve()}")
        image_out.save(outName)
...
def apply_to_scenes(
    ...
    anari_library,
    anari_device=None,
    anari_renderer="default",
    test_scenes="test_scenes",
    only_permutations=False,
    check_features=True,
    use_generator=True,
    tangent_gen=True,
    *args,
    ...
    if use_generator:
        try:
            sceneGenerator = anariCTSBackend.SceneGenerator(
                anari_library, anari_device, anari_logger
            )
        except Exception as e:
            print(e)
            return result
    ...
    for json_file_path in collected_scenes:
        ...
        if "test_scenes" in scene_location_parts:
            ...
            test_name = str(
                Path(
                    *(
                        scene_location_parts[
                            len(scene_location_parts) - test_scenes_index - 1 :
                        ]
                    )
                ).with_suffix("")
            )
        ...
        parsed_json = {}
        with open(json_file_path, "r", encoding="utf-8") as f, open(
            "default_test_scene.json", "r", encoding="utf-8"
        ) as defaultTestScene:
        ...
        if use_generator:
            ...
            if not all_features_available and check_features:
                ...
                print(f"Scene is not supported")
                    ...
                    if key == "anari_objects":
                        for [anariObjectName, array] in value.items():
                            for idx, item in enumerate(array):
                                ...
                                if "subtype" not in item:
                                    ...
                                    if (
                                        anariObjectName == "renderer"
                                        or anariObjectName == "instance"
                                        or anariObjectName == "group"
                                        or anariObjectName == "surface"
                                    ):
                                ...
                                if (
                                    anariObjectName == "geometry"
                                    and parsed_json["sceneParameters"][
                                        "geometrySubtype"
                                    ]
                                    != None
                                    and subtype
                                    != parsed_json["sceneParameters"]["geometrySubtype"]
                                ):
                                ...
                                sceneGenerator.createAnariObject(
                                    anariObjectName, subtype, ctsType
                                )
                                for [paramName, paramValue] in item.items():
                                    ...
                                    if isinstance(
                                        paramValue, str
                                    ) and paramValue.startswith("ref_"):
                                    ...
                                    elif isinstance(paramValue, list) and all(
                                        isinstance(elem, str)
                                        and elem.startswith("ref_")
                                        ...
                                        sceneGenerator.setGenericParameter(
                        ...
                        for [subkey, subvalue] in value.items():
                            ...
                            if key == "gltf" and subkey == "file":
                                ...
                                gltf = (
                                    ctsGLTF.loadGLB(pathToGltf)
                                    if subvalue.endswith(".glb")
                                    else ctsGLTF.loadGLTF(pathToGltf)
                                )
                                sceneGenerator.loadGLTF(
                                    json.dumps(gltf.json),
                                    gltf.buffers,
                                    gltf.images,
                                    tangent_gen,
                                )
        ...
        if "permutations" in parsed_json or (
            "variants" in parsed_json and not only_permutations
        ):
            ...
            if (
                "simplified_permutations" in parsed_json
                and parsed_json["simplified_permutations"]
            ):
            ...
            for permutation in permutations:
                ...
                for i in range(len(permutation)):
                    ...
                    if keys[i] in variant_keys:
                        ...
                        key = keys[i]
                        permutationString += (
                            f'{"_{}".format(simplifiedPermutationString)}'
                        )
                    ...
                    if use_generator:
                        ...
                        try:
                            if key.startswith("/anari_objects"):
                                pointer = key.split("/")
                                if isinstance(permutation[i], str) and permutation[
                                    i
                                ].startswith("ref_"):
                                    ref = permutation[i].split("_")
                                    sceneGenerator.setReferenceParameter(
                                        pointer[2],
                                        int(pointer[3]),
                                        pointer[4],
                                        ref[1],
                                ...
                                elif isinstance(permutation[i], list) and all(
                                    isinstance(elem, str) and elem.startswith("ref_")
                                    ...
                                    if permutation[i] is None:
                                        ...
                                        sceneGenerator.setGenericParameter(
                                            pointer[4], permutation[i]
                                        )
                            elif key.startswith("/"):
                                parameterName = key[1:].replace("/", "_")
                                sceneGenerator.setParameter(
                                    parameterName, permutation[i]
                                )
                                if key == "/gltf/file":
                                    os.chdir(Path(json_file_path).parent)
                ...
                result[test_name + permutationString + variantString] = func(
                    parsed_json,
                    sceneGenerator,
                    anari_renderer,
                    json_file_path,
...
def check_object_properties_helper(
    parsed_json,
    sceneGenerator,
    anari_renderer,
    scene_location,
    ...
    if "metaData" in parsed_json:
        ...
        if "world" not in ref_bounds:
            ...
            output += message
            return output, False
        check_output = check_bounding_boxes(
            ref_bounds["world"], bounds[0][0], tolerance
        )
        if check_output != "":
            message = terminalColors.error(
                f"Worlds bounds do not match!\n" + check_output
            )
            output += message
        if "instances" in ref_bounds:
            for i in range(len(ref_bounds["instances"])):
                check_output = check_bounding_boxes(
                    ref_bounds["instances"][i], bounds[1][i], tolerance
                )
                if check_output != "":
                    message = terminalColors.error(
                        f"Instance {i} bounds do not match!\n" + check_output
                    )
                    output += message
        if "groups" in ref_bounds:
            for i in range(len(ref_bounds["groups"])):
                check_output = check_bounding_boxes(
                    ref_bounds["groups"][i], bounds[2][i], tolerance
                )
                if check_output != "":
                    message = terminalColors.error(
                        f"Group {i} bounds do not match!\n" + check_output
                    )
                    output += message
    else:
...
def check_object_properties(
    anari_library, anari_device=None, test_scenes="test_scenes"
):
    return apply_to_scenes(
        check_object_properties_helper,
        anari_library,
        anari_device,
        "default",
        test_scenes,
...
def query_metadata(
    anari_library, type=None, subtype=None, skipParameters=False, info=False
):
    try:
        return anariCTSBackend.query_metadata(
            anari_library, type, subtype, skipParameters, info, anari_logger
        )
    except Exception as e:
        return str(e)
...
def create_report_for_scene(
    ...
    if sceneGenerator != None:
        ...
        property_check = check_object_properties_helper(
            parsed_json,
            sceneGenerator,
            anari_renderer,
            scene_location,
        ...
        if len(not_available_features) == 0:
            ...
            candidate_images = globImages(
                output / Path(test_name).parent, exclude_prefix=reference_prefix
            )
...
def create_report(
    ...
    try:
        merged_evaluations["device"] = (
            device
            if device != None
            else anariCTSBackend.getDefaultDeviceName(library, anari_logger)
        )
    except Exception as e:
        print(e)
        return
    ...
    # render test scenes
    result1 = apply_to_scenes(
        create_report_for_scene,
        library,
        device,
        renderer,
        test_scenes,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="ANARI CTS toolkit", formatter_class=argparse.RawTextHelpFormatter
    )
    ...
    libraryParser.add_argument(
        "--log_dir",
        default=None,
        type=Path,
        help="Directory in which ANARI.log file is saved. Defaults to working directory",
    )

    ignoreFeatureParser = argparse.ArgumentParser(add_help=False)
    ignoreFeatureParser.add_argument(
        "--ignore_features",
        action="store_true",
        help="Run tests even if feature is not supported",
    )

    noTangentGenParser = argparse.ArgumentParser(add_help=False)
    noTangentGenParser.add_argument(
        "--no_tangent_generation",
        action="store_true",
    ...
    sceneParser.add_argument(
        "-r", "--renderer", default="default", help="Renderer used to render the images"
    )
    sceneParser.add_argument(
        "-t",
        "--test_scenes",
        default="test_scenes",
        help="Folder with test scenes to test. Specify subfolder to test subsets. Also accepts glob patterns",
    )

    evaluationMethodParser = argparse.ArgumentParser(add_help=False)
    evaluationMethodParser.add_argument(
        "--comparison_methods",
        default=["ssim"],
        nargs="+",
        choices=["ssim", "psnr"],
        help="Specify all comparison methods to test against",
    )
    evaluationMethodParser.add_argument(
        "--thresholds",
        default=None,
        nargs="+",
        help="Specify custom thresholds for each comparison method",
    )
    evaluationMethodParser.add_argument(
        "-v",
        "--verbose_error",
        action="store_true",
        help="Include verbose infos of failed tests in report",
    )
    evaluationMethodParser.add_argument(
        "-vv",
        "--verbose_all",
        action="store_true",
        help="Include verbose infos of all tests in report",
    )
    ...
    evaluateScenesParser = subparsers.add_parser(
        "compare_images",
        description="Evaluates candidate renderings against reference renderings",
        parents=[evaluationMethodParser],
    )
    evaluateScenesParser.add_argument(
        "-t",
        "--test_scenes",
        default="test_scenes",
        help="Folder with test scenes which include the reference images",
    )
    evaluateScenesParser.add_argument(
        "--candidates",
        default="test_scenes",
        help="Path to folder containing the candidate images",
    )
    evaluateScenesParser.add_argument("-o", "--output", default=".", help="Output path")

    # command: query_features
    checkExtensionsParser = subparsers.add_parser(
        "query_features",
        parents=[deviceParser],
        description="Check which features are supported by the device",
    )

    # command: query_metadata
    queryMetadataParser = subparsers.add_parser(
        "query_metadata",
        parents=[libraryParser],
        description="Show metadata related to the specified library",
    )
    queryMetadataParser.add_argument(
        "--type", default=None, help="Only show parameters for objects of a type"
    )
    queryMetadataParser.add_argument(
        "--subtype", default=None, help="Only show parameters for objects of a subtype"
    )
    queryMetadataParser.add_argument(
        "--skipParameters", action="store_true", help="Skip parameter listing"
    )
    queryMetadataParser.add_argument(
        "--info",
        action="store_true",
        help="Show detailed information for each parameter",
    )

    # command: check_object_properties
    checkObjectPropertiesParser = subparsers.add_parser(
        "check_object_properties",
        parents=[deviceParser],
        description="Check if all properties are similar to the reference properties",
    )
    checkObjectPropertiesParser.add_argument(
        "-t",
        "--test_scenes",
        default="test_scenes",
        help="Folder with test scenes to test. Specify subfolder to test subsets. Also accepts glob patterns",
    ...
    create_reportParser = subparsers.add_parser(
        "create_report",
        parents=[
            sceneParser,
    ...
    queryInfoParser = subparsers.add_parser(
        "query_scenes_info",
        description="Lists information about the given test scene(s)",
    )
    queryInfoParser.add_argument(
        "-t",
        "--test_scenes",
        default="test_scenes",
        help="Folder with test scenes OR a test scene file to gather information from. Also accepts glob patterns",
    )
    queryInfoParser.add_argument(
        "--log_dir",
        default=None,
    ...
    for subparser in subparsers.choices:
        ...
        if subparsers.choices[subparser].description:
            subparsertext = (
                subparsertext.ljust(30) + subparsers.choices[subparser].description
            )
        subparsertext += "\n"
    ...
    verboseLevel = (
        2
        if "verbose_all" in args and args.verbose_all
        else 1 if "verbose_error" in args and args.verbose_error else 0
    )
    ...
    if args.command == "render_scenes":
        ...
        print(
            query_metadata(
                args.library, args.type, args.subtype, args.skipParameters, args.info
            )
        )
