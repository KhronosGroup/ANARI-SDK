...
connection_pointer connection_manager::connect(
...
connection_pointer connection_manager::get_or_connect(
    std::string const &host, unsigned short port)
{
...
void connection_manager::close_all()
{
  std::for_each(connections_.begin(),
      connections_.end(),
      [&](connection_pointer conn) { close(conn); });

  connections_.clear();
}

connection_pointer connection_manager::find(
    std::string const &host, unsigned short port)
{
  using boost::asio::ip::tcp;
...
void connection_manager::do_connect(
    std::string const &host, unsigned short port, handler h)
{
  connection_pointer conn(new connection(*this));
...
void connection_manager::handle_connect(
    boost::system::error_code const &e, connection_pointer conn, handler h)
{
  bool ok = h(conn, e);
...
void connection_manager::do_read(connection_pointer conn)
{
  ...
  boost::asio::async_read(conn->socket_,
      ...
      boost::bind(&connection_manager::handle_read_header,
          this,
          boost::asio::placeholders::error,
          message,
          conn));
...
void connection_manager::handle_read_header(boost::system::error_code const &e,
    ...
    connection_pointer conn)
{
  if (!e) {
    ...
    boost::asio::async_read(conn->socket_,
        ...
        boost::bind(&connection_manager::handle_read_data,
            this,
            boost::asio::placeholders::error,
            message,
            conn));
...
void connection_manager::handle_read_data(boost::system::error_code const &e,
    message_pointer message,
    connection_pointer conn)
{
  // Call the connection's slot
...
void connection_manager::do_write_0()
{
  ...
  buffers.push_back(boost::asio::const_buffer(
      &msg.second->header_, sizeof(msg.second->header_)));
  buffers.push_back(boost::asio::const_buffer(
      &msg.second->data_[0], msg.second->data_.size()));

  // Start the write operation.
  boost::asio::async_write(msg.first->socket_,
      buffers,
      boost::bind(&connection_manager::handle_write,
          this,
          boost::asio::placeholders::error,
          msg.second,
          msg.first));
...
void connection_manager::handle_write(boost::system::error_code const &e,
    message_pointer message,
    connection_pointer conn)
{
  // Call the connection's slot
