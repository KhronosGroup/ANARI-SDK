...
void print_info(ANARIDevice device, const char *objname, ANARIDataType objtype, const char *paramname, ANARIDataType paramtype, const char *indent) {
  int32_t *required = (int32_t*)anariGetParameterInfo(device, objname, objtype, paramname, paramtype, "required", ANARI_BOOL);
  if(required && *required) {
    printf("%srequired\n", indent);
  }

  const void *mem = anariGetParameterInfo(device, objname, objtype, paramname, paramtype, "default", paramtype);
  if(mem) {
  ...
  mem = anariGetParameterInfo(device, objname, objtype, paramname, paramtype, "minimum", paramtype);
  if(mem) {
  ...
  mem = anariGetParameterInfo(device, objname, objtype, paramname, paramtype, "maximum", paramtype);
  if(mem) {
  ...
  mem = anariGetParameterInfo(device, objname, objtype, paramname, paramtype, "use", ANARI_STRING);
  if(mem) {
    printf("%suse = " , indent);
  ...
  mem = anariGetParameterInfo(device, objname, objtype, paramname, paramtype, "value", ANARI_STRING_LIST);
  if(mem) {
  ...
  mem = anariGetParameterInfo(device, objname, objtype, paramname, paramtype, "value", ANARI_DATA_TYPE_LIST);
  if(mem) {
  ...
  mem = anariGetParameterInfo(device, objname, objtype, paramname, paramtype, "elementType", ANARI_DATA_TYPE_LIST);
  if(mem) {
    const ANARIDataType *list = (const ANARIDataType*)mem;
    printf("%selementType =\n", indent);
    for(;*list != ANARI_UNKNOWN;++list) {
  ...
  mem = anariGetParameterInfo(device, objname, objtype, paramname, paramtype, "description", ANARI_STRING);
  if(mem) {
    printf("%sdescription = \"%s\"\n", indent, (const char*)mem);
  }

  mem = anariGetParameterInfo(device, objname, objtype, paramname, paramtype, "sourceFeature", ANARI_STRING);
  if(mem) {
...
int main(int argc, const char **argv)
{
  ...
  for(int i = 0;devices[i];++i) {
    ...
    const char **channels = (const char**)anariGetObjectInfo(device, NULL, ANARI_FRAME, "channel", ANARI_STRING_LIST);
    if(channels) {
    ...
    if(!skipParameters) {
      ...
      for(size_t j = 0;j<sizeof(namedTypes)/sizeof(ANARIDataType);++j) {
        ...
        if(types) {
          for(int k = 0;types[k];++k){
            ...
            const ANARIParameter *params = (const ANARIParameter*)anariGetObjectInfo(device, types[k], namedTypes[j], "parameter", ANARI_PARAMETER_LIST);
            if(params) {
              for(int l = 0;params[l].name;++l){
                printf("         * %-25s %-25s\n", params[l].name, anari::toString(params[l].type));
                if(info) {
                  print_info(device, types[k], namedTypes[j], params[l].name, params[l].type, "            ");
                }
              }
            }
          }
      ...
      if(subtypeFilter == nullptr) {
        for(size_t j = 0;j<sizeof(anonymousTypes)/sizeof(ANARIDataType);++j) {
          ...
          const ANARIParameter *params = (const ANARIParameter*)anariGetObjectInfo(device, 0, anonymousTypes[j], "parameter", ANARI_PARAMETER_LIST);
          if(params) {
            for(int l = 0;params[l].name;++l){
              printf("         * %-25s %-25s\n", params[l].name, anari::toString(params[l].type));
              if(info) {
                print_info(device, nullptr, anonymousTypes[j], params[l].name, params[l].type, "            ");
              }
            }
          }
        }
