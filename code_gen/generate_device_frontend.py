...
class FrontendGenerator:
    ...
    def set_default(self, param, indentation):
        ...
        if isinstance(param["default"], list):
            code += indentation + basetype + " value[] = {" + ", ".join([str(x) for x in param["default"]]) + "};\n"
        elif isinstance(param["default"], str):
            code += indentation + "const char *value = \"" + param["default"] + "\";\n"
        else:
            code += indentation + basetype + " value[] = {" + str(param["default"]) + "};\n"
...
parser.add_argument("-d", "--device", dest="devicespec", help="The device json file.")
...
parser.add_argument("-l", "--header", dest="header", help="Header to be inserted at the top of files")
...
with open(args.prefix + "String.h", mode='w') as f:
    f.write(header)
...
with open(args.prefix + "String.cpp", mode='w') as f:
    f.write(header)
    f.write("// This file was generated by "+os.path.basename(__file__)+"\n")
    f.write("// Don't make changes to this directly\n\n")
...
with open(args.prefix + "Parameter.h", mode='w') as f:
    infile = (templatepath/"Parameter.h").open().read()
    ...
    infile = infile.replace("$template", "templates/Parameter.h")
    f.write(header)
    f.write(infile)

with open(args.prefix + "Objects.h", mode='w') as f:
    f.write(header)
    f.write("// This file was generated by "+os.path.basename(__file__)+"\n")
    f.write("// Don't make changes to this directly\n\n")
    f.write("#pragma once\n")
...
with open(args.prefix + "Objects.cpp", mode='w') as f:
    f.write(header)
    f.write("// This file was generated by "+os.path.basename(__file__)+"\n")
    f.write("// Don't make changes to this directly\n\n")
...
with open(args.prefix + "DeviceFactories.cpp", mode='w') as f:
    f.write(header)
    f.write("// This file was generated by "+os.path.basename(__file__)+"\n")
    f.write("// Don't make changes to this directly\n\n")
...
def generate_if_not_present(args, name, header, noprefix=False):
    filename = args.prefix + name
    if noprefix:
        filename = name
    if args.force and os.path.isfile(filename):
...
generate_if_not_present(args, "Object.h", header)
generate_if_not_present(args, "ArrayObjects.h", header)
generate_if_not_present(args, "ArrayObjects.cpp", header)
generate_if_not_present(args, "FrameObject.h", header)
generate_if_not_present(args, "FrameObject.cpp", header)
generate_if_not_present(args, "Device.h", header)
generate_if_not_present(args, "Device.cpp", header)
generate_if_not_present(args, "CMakeLists.txt", header.replace("//","#"), noprefix = True)

# create dummy files for these so cmake doesn't error
# when they aren't generated yet
if not os.path.isfile(args.prefix+"Queries.cpp"):
    with open(args.prefix+"Queries.cpp", mode='w') as f:
        pass

if not os.path.isfile(args.prefix+"DebugFactory.cpp"):
